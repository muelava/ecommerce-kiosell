...
?>
    <div title="<?php echo strtoupper($data['rajaongkir']['results'][$k]['name']); ?>" style="padding:10px;">
        <table border="1" class="table" style="font-size:.8em">
            <tr>
                <th>No.</th>
                <th>Jenis Layanan</th>
                <th>ETD</th>
...
<script type="text/javascript">
    $('.pilih_ongkir').on('click', function() {
        ...
        var total = tarif + <?= $harga_barang ?>;

        $('#keterangan').text("Harga Barang = " + "Rp. " + <?= $harga_barang ?> + " + Ongkir = " + tarif)

        $('.tombol-beli').text('Checkout Rp. ' + format_rupiah(total));
